...
@CommandHandler(DeleteOpportunityLocationCommand)
export class DeleteOpportunityLocationHandler implements ICommandHandler<DeleteOpportunityLocationCommand> {
  private readonly streamId = (command: DeleteOpportunityLocationCommand) =>
    `opportunity-location-${command.data.opportunity_id}-${command.data.location_id}`;

  private readonly handle: (command: DeleteOpportunityLocationCommand) => Promise<CommandHandlerResult>;
  ...
  async execute(command: DeleteOpportunityLocationCommand): Promise<any> {
