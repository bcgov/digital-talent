...
describe('DeleteOpportunityHandler', () => {
  let handler: DeleteOpportunityHandler;
  ...
  beforeEach(() => {
    ...
    handler = new DeleteOpportunityHandler(mockEventBus, mockEventStore as EventStoreDBClient);
  });

  it('should handle DeleteOpportunityCommand correctly', async () => {
    // eslint-disable-next-line @typescript-eslint/no-empty-function, func-names
    mockReadStream.mockImplementationOnce(async function* () {
      yield {
        event: {
          type: 'OpportunityCreatedEvent', // this mocks an opportunity being created earlier in eventstoredb
          data: JSON.stringify({
    ...
    // Mock for DeleteOpportunityInput
    const mockDeleteOpportunityInput: DeleteOpportunityInput = {
      id: 'd290f1ee-6c54-4b01-90e6-d701748f0851',
    };

    const command = new DeleteOpportunityCommand(mockDeleteOpportunityInput, mockMetadata);
