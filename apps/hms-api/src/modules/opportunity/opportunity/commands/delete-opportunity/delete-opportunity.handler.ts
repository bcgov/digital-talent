...
export class DeleteOpportunityHandler implements ICommandHandler<DeleteOpportunityCommand> {
  private readonly streamId = (command: DeleteOpportunityCommand) => `opportunity-${command.data.id}`;

  private readonly handle: (command: DeleteOpportunityCommand) => Promise<CommandHandlerResult>;
  ...
  async execute(command: DeleteOpportunityCommand): Promise<any> {
