...
@CommandHandler(UpdateOpportunityStateCommand)
export class UpdateOpportunityStateHandler implements ICommandHandler<UpdateOpportunityStateCommand> {
  private readonly streamId = (command: UpdateOpportunityStateCommand) => `opportunity-${command.data.id}`;

  private readonly handle: (command: UpdateOpportunityStateCommand) => Promise<CommandHandlerResult>;
  ...
  async execute(command: UpdateOpportunityStateCommand): Promise<any> {
