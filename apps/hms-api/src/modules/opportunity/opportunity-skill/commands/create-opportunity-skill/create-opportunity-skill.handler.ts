...
@CommandHandler(CreateOpportunitySkillCommand)
export class CreateOpportunitySkillHandler implements ICommandHandler<CreateOpportunitySkillCommand> {
  private readonly streamId = (command: CreateOpportunitySkillCommand) =>
    `opportunity-skill-${command.data.opportunity_id}-${command.data.skill_id}`;

  private readonly handle: (command: CreateOpportunitySkillCommand) => Promise<CommandHandlerResult>;
  ...
  async execute(command: CreateOpportunitySkillCommand): Promise<any> {
