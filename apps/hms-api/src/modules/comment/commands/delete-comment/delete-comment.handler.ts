...
export class DeleteCommentHandler implements ICommandHandler<DeleteCommentCommand> {
  private readonly streamId = (command: DeleteCommentCommand) => `comment-${command.data.id}`;

  private readonly handle: (command: DeleteCommentCommand) => Promise<CommandHandlerResult>;
