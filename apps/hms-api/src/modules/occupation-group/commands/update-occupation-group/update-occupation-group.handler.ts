...
@CommandHandler(UpdateOccupationGroupCommand)
export class UpdateOccupationGroupHandler implements ICommandHandler<UpdateOccupationGroupCommand> {
  private readonly streamId = (command: UpdateOccupationGroupCommand) => `occupation-group-${command.data.id}`;

  private readonly handle: (command: UpdateOccupationGroupCommand) => Promise<CommandHandlerResult>;
  ...
  async execute(command: UpdateOccupationGroupCommand): Promise<any> {
