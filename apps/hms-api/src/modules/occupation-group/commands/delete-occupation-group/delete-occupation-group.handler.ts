...
@CommandHandler(DeleteOccupationGroupCommand)
export class DeleteOccupationGroupHandler implements ICommandHandler<DeleteOccupationGroupCommand> {
  private readonly streamId = (command: DeleteOccupationGroupCommand) => `occupation-group-${command.data.id}`;

  private readonly handle: (command: DeleteOccupationGroupCommand) => Promise<CommandHandlerResult>;
  ...
  async execute(command: DeleteOccupationGroupCommand): Promise<any> {
