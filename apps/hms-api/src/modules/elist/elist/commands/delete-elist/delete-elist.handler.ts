...
@CommandHandler(DeleteElistCommand)
export class DeleteElistHandler implements ICommandHandler<DeleteElistCommand> {
  private readonly streamId = (command: DeleteElistCommand) => `elist-${command.data.id}`;

  private readonly handle: (command: DeleteElistCommand) => Promise<CommandHandlerResult>;
  ...
  async execute(command: DeleteElistCommand): Promise<any> {
