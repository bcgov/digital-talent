...
describe('ElistOfferDeletedEvent', () => {
  ...
  it('should correctly set data and metadata using constructor', () => {
    const event = new ElistOfferDeletedEvent(mockData, mockMetadata);

    expect(event.data).toEqual(mockData);
    expect(event.metadata).toEqual(mockMetadata);
  });

  it('should always set type to "ElistOfferDeletedEvent"', () => {
    const event = new ElistOfferDeletedEvent(mockData, mockMetadata);

    expect(event.type).toBe('ElistOfferDeletedEvent');
  });
});
