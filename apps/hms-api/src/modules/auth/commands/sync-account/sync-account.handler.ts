...
@CommandHandler(SyncAccountCommand)
export class SyncAccountHandler implements ICommandHandler<SyncAccountCommand> {
  private readonly streamId = (command: SyncAccountCommand) => `account-${command.data.id}`;

  private readonly handle: (command: SyncAccountCommand) => Promise<CommandHandlerResult>;
  ...
  async execute(command: SyncAccountCommand): Promise<any> {
