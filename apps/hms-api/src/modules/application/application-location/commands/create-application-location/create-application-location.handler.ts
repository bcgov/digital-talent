...
@CommandHandler(CreateApplicationLocationCommand)
export class CreateApplicationLocationHandler implements ICommandHandler<CreateApplicationLocationCommand> {
  private readonly streamId = (command: CreateApplicationLocationCommand) =>
    `application-location-${command.data.application_id}-${command.data.location_id}`;

  private readonly handle: (command: CreateApplicationLocationCommand) => Promise<CommandHandlerResult>;
  ...
  async execute(command: CreateApplicationLocationCommand): Promise<any> {
