...
describe('ApplicationLocationUpdatedEvent', () => {
  ...
  it('should correctly set data and metadata using constructor', () => {
    const event = new ApplicationLocationUpdatedEvent(mockData, mockMetadata);

    expect(event.data).toEqual(mockData);
    expect(event.metadata).toEqual(mockMetadata);
  });

  it('should always set type to "ApplicationLocationUpdatedEvent"', () => {
    const event = new ApplicationLocationUpdatedEvent(mockData, mockMetadata);

    expect(event.type).toBe('ApplicationLocationUpdatedEvent');
  });
});
