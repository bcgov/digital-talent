...
describe('ApplicationLocationDeletedEvent', () => {
  ...
  it('should correctly set data and metadata using constructor', () => {
    const event = new ApplicationLocationDeletedEvent(mockData, mockMetadata);

    expect(event.data).toEqual(mockData);
    expect(event.metadata).toEqual(mockMetadata);
  });

  it('should always set type to "ApplicationLocationDeletedEvent"', () => {
    const event = new ApplicationLocationDeletedEvent(mockData, mockMetadata);

    expect(event.type).toBe('ApplicationLocationDeletedEvent');
  });
});
