...
@CommandHandler(UpdateApplicationSkillCommand)
export class UpdateApplicationSkillHandler implements ICommandHandler<UpdateApplicationSkillCommand> {
  private readonly streamId = (command: UpdateApplicationSkillCommand) =>
    `application-skill-${command.data.application_id}-${command.data.skill_id}`;

  private readonly handle: (command: UpdateApplicationSkillCommand) => Promise<CommandHandlerResult>;
  ...
  async execute(command: UpdateApplicationSkillCommand): Promise<any> {
