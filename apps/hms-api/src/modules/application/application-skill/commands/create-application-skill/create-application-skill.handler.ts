...
@CommandHandler(CreateApplicationSkillCommand)
export class CreateApplicationSkillHandler implements ICommandHandler<CreateApplicationSkillCommand> {
  private readonly streamId = (command: CreateApplicationSkillCommand) =>
    `application-skill-${command.data.application_id}-${command.data.skill_id}`;

  private readonly handle: (command: CreateApplicationSkillCommand) => Promise<CommandHandlerResult>;
  ...
  async execute(command: CreateApplicationSkillCommand): Promise<any> {
