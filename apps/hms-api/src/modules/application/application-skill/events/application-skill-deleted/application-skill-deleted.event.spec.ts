...
describe('ApplicationSkillDeletedEvent', () => {
  ...
  it('should correctly set data and metadata using constructor', () => {
    const event = new ApplicationSkillDeletedEvent(mockData, mockMetadata);

    expect(event.data).toEqual(mockData);
    expect(event.metadata).toEqual(mockMetadata);
  });

  it('should always set type to "ApplicationSkillDeletedEvent"', () => {
    const event = new ApplicationSkillDeletedEvent(mockData, mockMetadata);

    expect(event.type).toBe('ApplicationSkillDeletedEvent');
  });
});
