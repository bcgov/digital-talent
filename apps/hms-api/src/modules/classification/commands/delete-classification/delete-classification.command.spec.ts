...
describe('DeleteClassificationCommand', () => {
  const mockData: DeleteClassificationInput = {
  ...
  it('should correctly set data and metadata using constructor', () => {
    const command = new DeleteClassificationCommand(mockData, mockMetadata);

    expect(command.data).toEqual(mockData);
    expect(command.metadata).toEqual(mockMetadata);
  });

  it('should always set type to "DeleteClassificationCommand"', () => {
    const command = new DeleteClassificationCommand(mockData, mockMetadata);

    expect(command.type).toBe('DeleteClassificationCommand');
  });
});
