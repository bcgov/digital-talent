...
import { DeletePositionHandler } from './delete-position.handler';

describe('DeletePositionHandler', () => {
  let handler: DeletePositionHandler;
  ...
  beforeEach(() => {
    ...
    handler = new DeletePositionHandler(mockEventBus, mockEventStore as EventStoreDBClient);
  });

  it('should handle DeletePositionCommand correctly', async () => {
    // eslint-disable-next-line @typescript-eslint/no-empty-function, func-names
    mockReadStream.mockImplementationOnce(async function* () {
      yield {
        event: {
          type: 'PositionCreatedEvent', // this mocks an position being created earlier in eventstoredb
          data: JSON.stringify({
    ...
    // Mock for DeletePositionInput
    const mockDeletePositionInput: DeletePositionInput = {
      id: 'd290f1ee-6c54-4b01-90e6-d701748f0851',
    };

    const command = new DeletePositionCommand(mockDeletePositionInput, mockMetadata);
