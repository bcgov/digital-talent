...
describe('ClassificationDeletedEvent', () => {
  ...
  it('should correctly set data and metadata using constructor', () => {
    const event = new ClassificationDeletedEvent(mockData, mockMetadata);

    expect(event.data).toEqual(mockData);
    expect(event.metadata).toEqual(mockMetadata);
  });

  it('should always set type to "ClassificationDeletedEvent"', () => {
    const event = new ClassificationDeletedEvent(mockData, mockMetadata);

    expect(event.type).toBe('ClassificationDeletedEvent');
  });
});
