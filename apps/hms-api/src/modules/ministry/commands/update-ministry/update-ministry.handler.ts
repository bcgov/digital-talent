...
@CommandHandler(UpdateMinistryCommand)
export class UpdateMinistryHandler implements ICommandHandler<UpdateMinistryCommand> {
  private readonly streamId = (command: UpdateMinistryCommand) => `ministry-${command.data.id}`;

  private readonly handle: (command: UpdateMinistryCommand) => Promise<CommandHandlerResult>;
  ...
  async execute(command: UpdateMinistryCommand): Promise<any> {
