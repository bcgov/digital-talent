...
describe('DeleteMinistryCommand', () => {
  ...
  it('should correctly set data and metadata using constructor', () => {
    const command = new DeleteMinistryCommand(mockData, mockMetadata);

    expect(command.data).toEqual(mockData);
    expect(command.metadata).toEqual(mockMetadata);
  });

  it('should always set type to "DeleteMinistryCommand"', () => {
    const command = new DeleteMinistryCommand(mockData, mockMetadata);

    expect(command.type).toBe('DeleteMinistryCommand');
  });
});
