...
describe('GridUpdatedEvent', () => {
  ...
  it('should correctly set data and metadata using constructor', () => {
    const event = new GridUpdatedEvent(mockData, mockMetadata);

    expect(event.data).toEqual(mockData);
    expect(event.metadata).toEqual(mockMetadata);
  });

  it('should always set type to "GridUpdatedEvent"', () => {
    const event = new GridUpdatedEvent(mockData, mockMetadata);

    expect(event.type).toBe('GridUpdatedEvent');
  });
});
