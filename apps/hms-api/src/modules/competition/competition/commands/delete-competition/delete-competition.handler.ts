...
export class DeleteCompetitionHandler implements ICommandHandler<DeleteCompetitionCommand> {
  private readonly streamId = (command: DeleteCompetitionCommand) => `competition-${command.data.id}`;

  private readonly handle: (command: DeleteCompetitionCommand) => Promise<CommandHandlerResult>;
  ...
  async execute(command: DeleteCompetitionCommand): Promise<any> {
