...
export class UpdateLocationHandler implements ICommandHandler<UpdateLocationCommand> {
  private readonly streamId = (command: UpdateLocationCommand) => `location-${command.data.id}`;

  private readonly handle: (command: UpdateLocationCommand) => Promise<CommandHandlerResult>;
  ...
  async execute(command: UpdateLocationCommand): Promise<any> {
