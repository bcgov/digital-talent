...
import { decider } from '../../user.decider';
...
export class SyncUserHandler implements ICommandHandler<SyncUserCommand> {
  private readonly streamId = (command: SyncUserCommand) => `user-${command.data.id}`;

  private readonly handle: (command: SyncUserCommand) => Promise<CommandHandlerResult>;
  ...
  async execute(command: SyncUserCommand): Promise<any> {
